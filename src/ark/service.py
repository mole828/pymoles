...
@router.get('/list.doc')
async def docs():
    return [doc async for doc in mongo.client.moles.doctor.find({}, {'_id': 0, 'token': 0})]
...
from src.ark.ArkNightsTask import map
...
@router.get('/base.out', include_in_schema=False)
async def baseOut():
    return {
        'doctor': [doc async for doc in mongo.client.moles.doctor.find({}, {'_id': 0})],
        'gacha': [gacha async for gacha in mongo.client.moles.gacha.find({}, {'_id': 0}).sort('ts', -1)],
        'diamond': [diamond async for diamond in mongo.client.moles.diamond.find({}, {'_id': 0}).sort('ts', -1)],
        'recent': [recent async for recent in mongo.client.moles.recent.find({}, {'_id': 0}).sort('ts', -1)],
    }
